...
namespace Vistony.Distribucion.Win
{
    public class SB1_MainMenuEvent
    {
        ...
        public void SB1_Application_MainMenuEvent(ref SAPbouiCOM.MenuEvent pVal, out bool BubbleEvent)
        {
            ...
            try
            {
                if (pVal.BeforeAction)
                {
                    switch (pVal.MenuUID)
                    {
                        #region Modulos/Call Reportes 
                        case AddonMenuItem.call_clientes:
                            {
                                onShowReporteClientes();

                            }
                            break;

                        case AddonMenuItem.call_semaforo:
                            {
                                onShowReporteSemaforo();

                            }
                            break;

                        case AddonMenuItem.call_articulos:
                            {
                                onShowReporteArticulos();

                            }
                            break;

                        case AddonMenuItem.call_interfase:
                            {
                                onShowInterfase();
                        ...
                        case AddonMenuItem.call_central:
                            {
                                onShowCentral();
        ...
        private void onShowReporteSemaforo()
        {
            try
            {
                udoSemaforo wizard = new udoSemaforo();
                wizard.Show();
        ...
        private void onShowReporteArticulos()
        {
            try
            {
                udoProductos wizard = new udoProductos();
                wizard.Show();
        ...
        private void onShowCentral()
        {
            try
            {
                udBPX wizard = new udBPX();
                wizard.Show();
        ...
        private void onShowReporteClientes()
        {
            try
            {
                AtencionClientes wizard = new AtencionClientes();
                wizard.Show();
